window.__ModuleLoader__.load({ id: 'dsh-scifi-theme', factory: (require) => {
  ...
  '  --dsw-alias-label-primary: #d2e8ee !important; --dsw-alias-label-primary-dimmed: #aad4dc !important; --dsw-alias-label-primary-foreground: #dcecf0 !important; --dsw-alias-label-primary-inverted: #001318 !important; --dsw-alias-label-primary-bluish: #7cd5e0 !important;',
  '  --dsw-alias-label-secondary: #8ac6da !important; --dsw-alias-label-tertiary: #5fa8c6 !important; --dsw-alias-label-caption: #4a7ead !important; --dsw-alias-label-dimmed: #3a6ea6 !important;',
  ...
  '#scifi-fx::after { content: ""; position: absolute; inset: -60px; background: radial-gradient(58% 42% at 84% 4%, rgba(0,255,249,0.26), transparent 68%), radial-gradient(52% 38% at 6% 96%, rgba(255,43,214,0.22), transparent 68%); animation: scifi-aurora 11s ease-in-out infinite alternate; will-change: transform, opacity, filter; filter: hue-rotate(0deg); }',
  '@keyframes scifi-aurora { from { opacity: 0.85; transform: translateY(0); filter: hue-rotate(0deg); } to { opacity: 1; transform: translateY(-16px); filter: hue-rotate(28deg); } }',
  /* studio panels: neon frame + HUD brackets + glitch title */
  '.gamespace-panel { position: relative; border: 1px solid transparent !important; background: linear-gradient(160deg, rgba(0,255,249,0.05), rgba(3,8,22,0.92) 34%) padding-box, conic-gradient(from var(--gs-angle, 0deg), rgba(0,255,249,0.95), rgba(0,255,249,0.12) 22%, rgba(255,43,214,0.85) 50%, rgba(0,255,249,0.12) 78%, rgba(0,255,249,0.95)) border-box !important; -webkit-backdrop-filter: blur(12px); backdrop-filter: blur(12px); animation: gs-spin 7s linear infinite; box-shadow: 0 0 22px rgba(0,255,249,0.20) !important; }',
